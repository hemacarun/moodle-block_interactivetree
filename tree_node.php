    ...
    try {
        ...
        switch ($opertaion) {
            ...
            case 'get_node':
                ...
                $rslt = array();
                foreach ($temp as $v) {
                    $treeinfo = $DB->get_record('block_interactivetree_data', array(
                        'id' => $v->id
                    ));
            ...
            case "get_content":
                ...
                if (count($node) > 1) {
                    ...
                    $temp = $fs->get_node((int) $node[0], array(
                        'with_path' => true
                    ));
                    $rslt = array(
            ...
            case 'create_node':
                if (has_capability('block/interactivetree:manage', $blockcontext)) {
                    $node = isset($id) && $id !== '#' ? (int) $id : 0;
                    $temp = $fs->createnode($node, isset($position) ? (int) $position : 0, array(
                        'nm' => isset($text) ? $text : 'New node'
                    ));
                    $rslt = array(
                        'id' => $temp
                    );
            ...
            case 'rename_node':
                if (has_capability('block/interactivetree:manage', $blockcontext)) {
                    $node = isset($id) && $id !== '#' ? (int) $id : 0;
                    $rslt = $fs->renamenode($node, array(
                        'nm' => isset($text) ? $text : 'Renamed node'
                    ));
                }
                break;
